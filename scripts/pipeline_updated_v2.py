...
class RuntimeTracker:
    ...
    def stop(self, name):
        ...
        self.results[name] = round(elapsed, 3)
        print(f"[Runtime] {name}: {elapsed:.3f} seconds")
        return elapsed
...
def run_script(script_name, script_path, args=None, runtime=None):
    if args is None:
        args = []
    ...
    print(f"[Runtime] {script_name}: {elapsed:.3f} seconds")

    if runtime is not None:
        runtime.results[script_name] = round(elapsed, 3)
...
def main():
    ...
    runtime.start("main_function_total")
    ...
    runtime_path = os.path.join(default_dir, "runtime_report.json")

    runtime.stop("setup_paths")

    # ---------------------------
    # 2. Selection Phase
    # ---------------------------

    runtime.start("selection_phase")
    ...
    runtime.stop("selection_phase")

    # ---------------------------
    # 3. Read Metadata
    # ---------------------------

    runtime.start("read_metadata")
    ...
    runtime.stop("read_metadata")

    # ---------------------------
    # 4. Header and Prefix Logic
    # ---------------------------

    runtime.start("header_prefix_date_logic")
    ...
    runtime.stop("header_prefix_date_logic")

    # ---------------------------
    # 5. First Pass: Find Spectral Headers
    # ---------------------------

    runtime.start("find_spectral_headers")
    ...
    runtime.stop("find_spectral_headers")

    # ---------------------------
    # 6. Second Pass: Process All Rows
    # ---------------------------

    runtime.start("process_all_rows")
    ...
    runtime.stop("process_all_rows")

    # ---------------------------
    # 7. Write CSV, Log, Summary and Runtime
    # ---------------------------

    runtime.start("write_outputs")
    ...
    runtime.stop("write_outputs")
    runtime.stop("main_function_total")
...
if __name__ == "__main__":
    ...
    heatmap_script = os.path.join(script_dir, "visualise_heatmap.py")
    measurement_script = os.path.join(script_dir, "visualise_measurement.py")
    ...
    try:
        ...
        print("\nA follow-up script failed.")
        print(f"Failed command: {e.cmd}")
        print(f"Exit code: {e.returncode}")

        runtime.results["pipeline_failed"] = True
        runtime.results["failed_command"] = str(e.cmd)
        runtime.results["failed_exit_code"] = e.returncode
        runtime.save(runtime_path)

        sys.exit(e.returncode)

    full_elapsed = time.perf_counter() - pipeline_total_start

    runtime.results["full_pipeline_total"] = round(full_elapsed, 3)
    runtime.results["pipeline_failed"] = False
    runtime.save(runtime_path)

    print(f"[Runtime] full_pipeline_total: {full_elapsed:.3f} seconds")
