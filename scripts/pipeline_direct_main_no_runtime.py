...
def run_module_main(module_name, display_name, args=None):
    ...
    This avoids starting a new Python process.
    The module is imported only when needed, so report.py is loaded after
    summary.json has already been generated by the main pipeline.
    """
    if args is None:
        args = []

    print(f"\nRunning {display_name} through main() ...")

    module = importlib.import_module(module_name)
    main_func = getattr(module, "main", None)

    if not callable(main_func):
        raise AttributeError(f"{display_name} does not define a callable main() function.")
    ...
    if args and (accepts_varargs or positional_capacity >= len(args)):
        main_func(*args)
    else:
        main_func()
